  ...
  if (isect) {
    if ((isect.material.kr || isect.material.kt) && (depth < maxDepth)) {
      if (isect.material.kr) {
        ...
        color.add(raytracing(reflectedRay, depth + 1).multiply(isect.material.kr))
      }
      if (isect.material.kt) {
        let refractedRay = new Ray(isect.position, refract(ray.d.clone(), isect.normal, isect.material.ior))
        color.add(raytracing(refractedRay, depth + 1).multiply(isect.material.kt))
      }
    } else {
...
function shading (ray, isect) {
  ...
  for (let i = 0; i < lights.length; i++) {
    ...
    let isectShadow = rayIntersectScene(shadowRay)
