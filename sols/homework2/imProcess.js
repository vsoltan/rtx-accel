...
//#begin
	...
	for(let i=0; i<input.width*input.height; i++)	{
		let idx=i*4;
		let gray = (ip[idx+0]+ip[idx+1]+ip[idx+2])/3;
		op[idx+0] = pixelClamp(saturation*ip[idx+0]+(1-saturation)*gray); // red
		op[idx+1] = pixelClamp(saturation*ip[idx+1]+(1-saturation)*gray); // green
		op[idx+2] = pixelClamp(saturation*ip[idx+2]+(1-saturation)*gray); // blue
	}
//#end
}
...
//#begin
	let s = sharpness;
	let lkernel = [[0, -s, 0], [-s, 1+4*s, -s], [0, -s, 0]];
	filterImage(input, output, lkernel);
//#end	
}
...
//#begin
	...
	for(let y=0; y<input.height; y++) {
		for(let x=0; x<input.width; x++) {
			...
			let gray = (ip[idx+0]+ip[idx+1]+ip[idx+2])/3;
			gray = (gray > 127) ? 255 : 0;
			op[idx+0]=op[idx+1]=op[idx+2]=gray;
		}
	}
//#end
}
...
//#begin
	...
	for(let y=0; y<input.height; y++) {
		for(let x=0; x<input.width; x++) {
			let idx = (y*input.width+x)*4;
			let gray = (ip[idx+0]+ip[idx+1]+ip[idx+2])/3;
			gray = (gray > Math.random()*255 ) ? 255 : 0;
			op[idx+0]=op[idx+1]=op[idx+2]=gray;
		}
	}
//#end
...
//#begin
	...
	for(let y=0; y<input.height; y++) {
		for(let x=0; x<input.width; x++) {
			let idx = (y*input.width+x)*4;
			let gray = (ip[idx+0]+ip[idx+1]+ip[idx+2])/3;
			gray = (gray > bayers[x%4][y%4]*255) ? 255 : 0;
			op[idx+0]=op[idx+1]=op[idx+2]=gray;
		}
	}
//#end
...
//#begin
	// compute squared image norms
	let r_norms = [], g_norms = [], b_norms = [];

	for(let i=0; i < num; i++) {
		r_norms[i]=g_norms[i]=b_norms[i]=0;
		//mimages[i].loadPixels();

		for(let j=0; j < 4*w*h; j+=4) {
			let r = mimages[i].pixels[j+0];
			let g = mimages[i].pixels[j+1];
			let b = mimages[i].pixels[j+2];
...
//#begin		
			let best_match = -1;
			let max_corr = -1;
			let r_scale = 1.0, g_scale = 1.0, b_scale = 1.0;
			
			for(let i=0; i < num; i++) {
				let r_product = 0.0, g_product = 0.0, b_product = 0.0;
				let l2 = 0.0;

				let j=0;
				for (let s=0;s<h;s++) {
					for(let t=0;t<w;t++,j+=4) {
						let r = mimages[i].pixels[j+0];
						let g = mimages[i].pixels[j+1];
						let b = mimages[i].pixels[j+2];
						let oidx = ((y+s)*width+(x+t))*4;
						let rr = input.pixels[oidx+0];
						let gg = input.pixels[oidx+1];
						let bb = input.pixels[oidx+2];
				...
				let corr = r_product*r_product/r_norms[i] +
						g_product*g_product/g_norms[i] +
						b_product*b_product/b_norms[i];

				corr *= (1 + Math.random() * 0.5);

				if (corr > max_corr) {
					max_corr = corr;
					best_match = i;
					r_scale = r_product/r_norms[i];
					g_scale = g_product/g_norms[i];
					b_scale = b_product/b_norms[i];
				}
				/*
				corr = (r_product+g_product+b_product);
				if(max_corr < 0) { max_corr = corr; best_match = i; }
				else {
					if(corr<max_corr) {
						max_corr = corr;
						best_match = i;
						r_scale = g_scale = b_scale =1;
					}
				}*/
			}
			...
			for (let s=0;s<h;s++) {
				for(let t=0;t<w;t++,j+=4) {
					let oidx = ((y+s)*width+(x+t))*4;				
					let ri = mimages[best_match].pixels[j+0] * r_scale;
					let gi = mimages[best_match].pixels[j+1] * g_scale;
					let bi = mimages[best_match].pixels[j+2] * b_scale;

					output.pixels[oidx+0] = pixelClamp(ri);
					output.pixels[oidx+1] = pixelClamp(gi);
					output.pixels[oidx+2] = pixelClamp(bi);
				}
			}
//#end
		}
