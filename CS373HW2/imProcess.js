...
function adjustContrast(input, output, contrast) {
	...
	for (let i = 0; i < input.width * input.height; i++) {
		let idx = i * 4;
		op[idx + 0] = pixelClamp(ip[idx + 0] * contrast + 127 * (1 - contrast));
		op[idx + 1] = pixelClamp(ip[idx + 1] * contrast + 127 * (1 - contrast));
		op[idx + 2] = pixelClamp(ip[idx + 2] * contrast + 127 * (1 - contrast));
	}
}
 ...
 * =================================================== */
 ...
 function setWhite(image, index) {
	 image[index + 0] = 255;
	 image[index + 1] = 255;
	 image[index + 2] = 255;
 }

 function setBlack(image, index) {
	 image[index + 0] = 0;
	 image[index + 1] = 0;
	 image[index + 2] = 0;
 }
...
function uniformQuantization(input, output) {
	...
	for (let i = 0; i < input.width * input.height; i++) {
		...
		grayScale > 127 ? setWhite(op, idx) : setBlack(op, idx);
	}
}
...
function randomDither(input, output) {
	...
	for (let i = 0; i < input.width * input.height; i++) {
		...
		grayScale > Math.random() * 255 ? setWhite(op, idx) : setBlack(op, idx);
	}
}
...
function orderedDither(input, output) {
	...
	for(let y = 0; y < input.height; y++) {
  	for(let x = 0; x < input.width; x++) {
			...
			grayScale > e ? setWhite(op, idx) : setBlack(op, idx);
		}
  }
}
...
function imageMosaic(input, output, mosaic_name) {
  ...
  (function chunk() {
		for (x = 0; x <= width - w; x += w) { // x loop
			...
			for (let k = 0; k < mimages.length; k++) { // candidates loop

				let d_r = d_g = d_b = d = 0;
				let d_r_n = d_g_n = d_b_n = 0; // numerator
				let d_r_d = d_g_d = d_b_d = 0; // denominator

				let mimage = mimages[k];

				for (let mh = 0; mh < h; mh++) {
					for (let mw = 0; mw < w; mw++) {

						let mp = mimage.pixels;
						let idx = ((y + mh) * width + (x + mw)) * 4; // index of input image
						let idxm = (mh * w + mw) * 4; // index of candidate image

						d_r_n += (ip[idx + 0] * mp[idxm + 0]);
						d_g_n += (ip[idx + 1] * mp[idxm + 1]);
						d_b_n += (ip[idx + 2] * mp[idxm + 2]);
				...
				d = -1 * (d_r + d_g + d_b) * getRandomNumber(1.0, 2.0);
			...
			let min_image = mimages[min_k].pixels;

			// paste in best choice image
			for (let i = x; i < x + w; i++) {
				for (let j = y; j < y + h; j++) {
					...
					op[odx + 0] = pixelClamp(min_image[mdx + 0] * a_r);
					op[odx + 1] = pixelClamp(min_image[mdx + 1] * a_g);
					op[odx + 2] = pixelClamp(min_image[mdx + 2] * a_b);
				}
			}
		}
