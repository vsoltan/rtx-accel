...
// blue 
var material = new THREE.MeshStandardMaterial( {color: new THREE.Color(0, 0, 1), side: THREE.DoubleSide} );
...
var square_geometry = new THREE.Geometry();

square_geometry.vertices.push(
...
square_geometry.faces.push( new THREE.Face3( 0,1,2 ) );
square_geometry.faces.push( new THREE.Face3( 0,2,3 ) );
square_geometry.computeFaceNormals();
var square = new THREE.Mesh(square_geometry, material);

shapes.push(square);

// CUSTOM SHAPE: RING

var ring_geometry = new THREE.RingGeometry(0.5, 1, 10, 4, 0, Math.PI * 2);
var ring = new THREE.Mesh(ring_geometry, material);
shapes.push(ring);
...
function render() {
   ...
   * Write your answer here in 1-2 sentences:
	 *
	 * when the image is rendered, the program uses a raycaster to construct a vector (ray) from the mouse position
	 * to the current scene and determines if that ray interesects with any of the scene components. The method
	 * returns all objects penetrated by the ray; if the length of the resulting array is greater than 0 then the shape was clicked.
	 *
   *********************************/
