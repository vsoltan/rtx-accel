...
function scalarMult(pt, c) {
	return {'x': pt.x * c, 'y': pt.y * c};
}

function addPts(pt1, pt2) {
	return {'x': pt1.x + pt2.x, 'y': pt1.y + pt2.y};
}

function derivative(pt) {
...
// recursive function to generate sampled curve points
function computePoint(pointList, t) {
	...
	for (let i = 0; i < pointList.length - 1; i++) {
		sampledPoints.push(interpolatePoints(pointList[i], pointList[i + 1], t));
	}
...
function computeCubicPoint(pointList, t) {
	...
	let A = (-(t ** 3) + 3 * t ** 2 - 3 * t + 1) / 6;
	let B = (3 * t ** 3 - 6 * t ** 2 + 4) / 6;
	let C = (-3 * t ** 3 + 3 * t ** 2 + 3 * t + 1) / 6;
	let D = t ** 3 / 6;

	newx = A * pointList[0].x + B * pointList[1].x + C * pointList[2].x + D * pointList[3].x;
	newy = A * pointList[0].y + B * pointList[1].y + C * pointList[2].y + D * pointList[3].y;

	return {'x': newx, 'y': newy};
}

function circularSlice(a, start, size) {
	let sliced = [];
	let circularIndex = start % a.length;

	for (let i = 0; i < size; i++) {
		sliced.push(a[circularIndex]);
		circularIndex = (circularIndex + 1) % a.length;
	}
	return sliced;
}
...
function computeBSpline() {
	...
	// loop through all control points like a circular queue
	if (closedCurve) {
		for (let i = 0; i < controlPts.length; i++) {
			let pointList = circularSlice(controlPts, i, 4);
			for (let i = 0; i <= nSegment; i++) {
				let t = i / nSegment;
				curvePts.push(computeCubicPoint(pointList, t));
			}
		}
	} else {
		for (let i = 0; i < controlPts.length - 3; i++) {
			let pointList = controlPts.slice(i, i + 4);
			for (let i = 0; i <= nSegment; i++) {
				let t = i / nSegment;
				curvePts.push(computeCubicPoint(pointList, t));
			}
...
function computeHermitePoint(pointList, t) {
...
function computeHermite() {
	...
	if (closedCurve) {
		for (let i = 0; i < controlPts.length; i++) {
			let pointList = circularSlice(controlPts, i, 2);
			for (let i = 0; i <= nSegment; i++) {
				let t = i / nSegment;
				curvePts.push(computeHermitePoint(pointList, t));
			}
		}
	} else {
		for (let i = 0; i < controlPts.length - 1; i++) {
			let pointList = controlPts.slice(i, i + 2);
			for (let i = 0; i <= nSegment; i++) {
				let t = i / nSegment;
				curvePts.push(computeHermitePoint(pointList, t));
			}
